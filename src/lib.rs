                     ...
                     stride: uint, starting_idx: uint, factors: &[uint])
{
    ...
    if factors.len() == 1
    {
        cooley_tukey_base(signal.slice_from(starting_idx), spectrum.slice_from_mut(starting_idx), stride, stride);
    }
    else
    {
        let n1 = factors[0];
        ...
        multiply_by_twiddles(temp.slice_from_mut(starting_idx), stride, n1, n2);

        for (i, chunk) in temp.slice_from_mut(starting_idx).chunks(stride * n1).enumerate()
        {
            cooley_tukey_base(chunk, spectrum.slice_from_mut(i * stride + starting_idx), stride, n2 * stride);
        }
